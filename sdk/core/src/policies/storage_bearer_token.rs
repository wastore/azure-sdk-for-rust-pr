...
impl BearerTokenCredentialPolicy {
    ...
    fn update_token(&self, new_access_token: &AccessToken) {
        let mut current_access_token = self.access_token.lock().unwrap();
        *current_access_token = Some(new_access_token.clone());
    ...
    fn access_token(&self) -> Result<String, &'static str> {
        let current_access_token = self.access_token.lock().unwrap().clone();
